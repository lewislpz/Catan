...
describe("robber blocking", () => {
  it("prevents production on the blocked tile", () => {
    const state = makeEngineState("robber-seed");
    ...
    state.game.robber.tileId = tile.id;
    ...
    if (productionEvent?.type === "RESOURCES_PRODUCED") {
      expect(productionEvent.allocations).toHaveLength(0);
    }
  });
});
